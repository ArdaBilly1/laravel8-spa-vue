								...
								<table id="example2" class="table table-bordered table-hover">
									...
									<tbody>
										<tr v-for="(library, index) in libraries"  v-bind:key="library.id" >
											...
											<td>{{ library.penerbit }}</td>
											<td>{{ library.status }}</td>
											<td>
												<router-link  :to="{ name: 'item-product-edit', params: {id:library.id}}"  class="btn btn-outline-success">
													<i class="fa fa-edit"></i>
												</router-link>
												<button v-on:click="destroy(library.id,index)" type="button" class="btn btn-outline-danger">
